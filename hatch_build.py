...
`pip install pygpeople` works with no user setup.

So the build copies it in from outside the tree, trying in order:

  1. $PYGPEOPLE_CLIENT_SECRET  -- for release automation (CI secret -> file)
  2. ~/.config/pygpeople/client_secret.json  -- the usual developer location
  3. src/pygpeople/client_secret.json  -- already in place, left alone
...
CLIENT_SECRET = os.path.join("src", "pygpeople", "client_secret.json")
CONFIG_LOCATION = os.path.expanduser("~/.config/pygpeople/client_secret.json")
...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version, build_data):
        ...
        for source in (os.environ.get("PYGPEOPLE_CLIENT_SECRET"), CONFIG_LOCATION):
        ...
        raise RuntimeError(
            ...
            f"{CONFIG_LOCATION},\n"
            "or set PYGPEOPLE_CLIENT_SECRET to its path, before building."
        )
